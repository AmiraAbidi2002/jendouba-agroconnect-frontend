...
// Hook personnalisé doit être défini au niveau supérieur
 ...
 useEffect(() => {
  ...
  return (
    <div className="flex min-h-screen bg-[#f0f7f5]"> 
      {/* Sidebar desktop */}
      {!isMobile && (
        <aside className="fixed top-0 left-0 h-full w-64 bg-[#1d4c43] text-white p-6 z-40">
          <img
      ...
      {isMobile && (
        <>
          ...
          {isSidebarOpen && (
            <aside
              className="fixed top-0 left-0 h-full w-64 bg-[#1d4c43] text-white p-6 z-40"
            >
              <img
                src="/src/assets/jendouba_agroconnect_logo.png"
                alt="Logo AgroConnect"
      ...
      {/* Main Content */}
      <main className={`flex-1 p-8 overflow-auto bg-[#f0f7f5] ${!isMobile ? 'md:ml-64' : ''} relative z-10`}> 
