...
export default function FarmerDashboard() {
  ...
  useEffect(() => {
    const fetchFarm = async () => {
      ...
      try {
        const data = await getMyFarm(user.token); // <-- passer le token ici
  ...
  // Ferme(s) à afficher : soit toutes, soit juste la mienne
  ...
  const fetchCrops = async () => {
    try {
      const userId = Number(user?.id ?? user?.sub); // conversion string → number
      if (!userId) return;
