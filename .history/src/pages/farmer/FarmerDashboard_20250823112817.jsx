...
export default function FarmerDashboard() {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        console.log("Buyers reçus:", res.data);
        setBuyers(res.data);})
      .catch((err) => console.error(err));
  }, []);
...
console.log("User object:", user); // ✅ Vérifiez la structure
// ✅ VÉRIFICATION DU RÔLE - EMPÊCHE L'ACCÈS AUX NON-FARMERS
  ...
  const fetchCrops = async () => {
    try {
      // ✅ Assurez-vous que user.id existe et est numérique
    if (!user.id) {
      console.error("User ID is missing");
      return;
    }
        console.log("Fetching crops for user ID:", user.id);
      const data = await getCropsByFarmer(user.id); // user.id numérique
    ...
    if (error.response?.status === 404) {
      console.error("Endpoint not found. Check backend route.");
    }
    }
  };
          ...
          <div className={cardClass}>
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <button
                ...
                onClick={() => {
                  setEditingCrop(null); // Réinitialiser
                  setShowForm(true);    // Ouvrir le modal
                }}
              >
                Add New Crop
              </button>
