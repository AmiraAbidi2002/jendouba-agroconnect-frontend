...
export default function FarmerDashboard() {
  ...
  // Fonction pour récupérer MA ferme
  const fetchMyFarm = async () => {
    try {
      console.log("Chargement de ma ferme...");
      const farmData = await getMyFarm();
      console.log("Données de ma ferme:", farmData);
      setmineFarm(farmData);
    } catch (error) {
      console.error("Erreur lors du chargement de ma ferme:", error);
    }
  };
      ...
      <main className="flex-1 p-8 overflow-auto">
        ...
        {activeSection === "Farm Location" && (
          <div className={cardClass}>
            ...
            {!mineFarm && !showAllFarms && (
              <div className="text-center py-8 text-gray-500">
                Cliquez sur "Voir ma ferme" pour afficher votre ferme
              </div>
            )}
            
            <FarmMap farms={showAllFarms ? farms : (mineFarm ? [mineFarm] : [])} />
            
            {mineFarm && !showAllFarms && (
              <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                <h3 className="font-semibold mb-2">Détails de votre ferme:</h3>
                <p><strong>Nom:</strong> {mineFarm.farmer_name}</p>
                <p><strong>Localisation:</strong> {mineFarm.location}</p>
                <p><strong>Nombre de cultures:</strong> {mineFarm.crops?.length || 0}</p>
              </div>
            )}
          </div>
        )}
