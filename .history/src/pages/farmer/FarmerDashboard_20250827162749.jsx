...
const fetchCrops = useCallback(async () => {
  try {
    ...
    let data;
    if (viewMode === "mine") {
      data = await getCropsByFarmer(user.id);
    } else {
...
{activeSection === "Profile" && (
  <div className={cardClass}>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-[#1d4c43] mb-1">
            Full Name
          </label>
          {isEditingProfile ? (
            <input
  ...
  <FiMail className="mr-2 text-[#1d4c43]" /> {/* couleur appliquée à l'icône */}
  <label className="text-sm font-medium text-[#1d4c43]">
    Email
  </label>
        ...
        <div>
          <label className="block text-sm font-medium text-[#1d4c43] mb-1 flex items-center">
