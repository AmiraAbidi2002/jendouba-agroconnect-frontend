...
import {
  ...
  FiMap,
  FiX,
  FiMenu // Ajouté pour le menu burger mobile
} from "react-icons/fi";
...
// Load crops when the active section changes 
 useEffect(() => {
  ...
  const menuItems = [
    { name: "Profile", icon: <FiUser className="mr-2 sm:mr-3" /> },
    { name: "Crops", icon: <FiPackage className="mr-2 sm:mr-3" /> },
    { name: "Farm Location", icon: <FiMapPin className="mr-2 sm:mr-3" /> },
    { name: "Weather Forecast", icon: <FiCloud className="mr-2 sm:mr-3" /> },
    { name: "Messages", icon: <FiMessageSquare className="mr-2 sm:mr-3" /> },
  ];
  ...
  return (
    <div className="flex min-h-screen bg-[#f0f7f5]"> 
      ...
      <button
        className="fixed top-4 left-4 z-50 lg:hidden bg-[#1d4c43] text-white p-2 rounded-lg shadow-lg"
      ...
      `}>
        ...
        <button
          className="self-end mb-4 lg:hidden text-white"
          onClick={() => setIsMobileMenuOpen(false)}
        >
          <FiX size={24} />
        </button>
        ...
        <nav className="flex-1">
          {menuItems.map((item) => (
            <button
              key={item.name}
              className={`flex items-center w-full mb-2 px-3 sm:px-4 py-2 sm:py-3 transition-colors rounded-lg text-sm sm:text-base ${
        ...
        <div className="mt-auto pt-4 border-t border-[#2a5c45]">
          <p className="text-xs sm:text-sm opacity-75 mb-3">Logged in as Farmer</p>
          <button
            onClick={logout}
            className="w-full flex items-center justify-center bg-red-500 hover:bg-red-600 text-white py-2 px-3 sm:px-4 rounded-lg transition text-sm sm:text-base"
          >
      ...
      <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-auto bg-[#f0f7f5] lg:ml-0"> 
          ...
          <div className={`${cardClass} profile-section`}>
                ...
                {isEditingProfile ? (
                  <>
                    <button
                      ...
                      className="bg-[#1d4c43] text-white px-4 py-2 rounded hover:bg-[#2a5c45] flex items-center justify-center text-sm sm:text-base"
                    >
                      <FiSave className="mr-2" /> Save
                    </button>
                    <button
                      onClick={handleProfileCancel}
                      className="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600 flex items-center justify-center disabled:opacity-50 text-sm sm:text-base"
                    >
                      Cancel
                    </button>
                  </>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
              ...
              <div className="space-y-4">
                {/* User ID */}
                <div>
                  <label className="profile-label flex items-center text-sm sm:text-base font-medium text-gray-700 mb-1">
                    <FiUser className="mr-2" /> User ID
                  </label>
                  <p className="text-gray-900 bg-[#FEF2F2] p-2 sm:p-3 rounded-lg text-sm sm:text-base">{user.id}</p>
                </div>
                ...
                <div>
                  ...
                  {isEditingProfile ? (
                    <input
                      ...
                      className="w-full rounded-lg px-3 sm:px-4 py-2 border border-[#FEF2F2] bg-[#FEF2F2] focus:outline-none focus:ring-2 focus:ring-red-300 text-sm sm:text-base"
                    />
                  ) : (
                    <p className="text-gray-900 bg-[#FEF2F2] p-2 sm:p-3 rounded-lg text-sm sm:text-base">{profileData.name}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {isEditingProfile ? (
                    ...
                    <p className="text-gray-900 bg-[#FEF2F2] p-2 sm:p-3 rounded-lg text-sm sm:text-base">{profileData.email}</p>
                  )}
                </div>
              </div>

              {/* Right: Farm Location - Ajusté pour mobile */}
              {mineFarm && (
                <div className="space-y-4">
                  <h2 className="text-lg sm:text-xl font-bold text-[#1d4c43]">My Farm Location</h2>
                  <div className="h-48 sm:h-56 lg:h-64 w-full border rounded-lg overflow-hidden">
                    <FarmMap 
                      farms={[mineFarm]} 
                      editable={false} 
                    />
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-4">
              ...
              <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                ...
                <div className="flex gap-2">
                  <button
                    className={`px-3 py-2 rounded text-sm sm:text-base flex-1 sm:flex-none ${
                      viewMode === "mine"
                        ? "bg-[#1d4c43] text-white"
                ...
                {viewMode === "mine" && (
                  <button
                    className="bg-[#1d4c43] text-white px-3 py-2 rounded hover:bg-[#2a5c45] text-sm sm:text-base whitespace-nowrap"
                    onClick={() => {
                      setEditingCrop(null);
                      setShowForm(true);
                    }}
            ...
            {!showForm &&
              (crops.length > 0 ? (
                    ...
                    <div className="overflow-hidden border border-gray-200 rounded-lg">
                      <table className="min-w-full bg-white">
                        <thead className="bg-[#1d4c43] text-white">
                          <tr>
                            <th className="py-2 sm:py-3 px-2 sm:px-4 text-left text-xs sm:text-sm">ID</th>
                            <th className="py-2 sm:py-3 px-2 sm:px-4 text-left text-xs sm:text-sm">Name</th>
                            <th className="py-2 sm:py-3 px-2 sm:px-4 text-left text-xs sm:text-sm">Farmer</th>
                            <th className="py-2 sm:py-3 px-2 sm:px-4 text-left text-xs sm:text-sm">Type</th>
                            <th className="py-2 sm:py-3 px-2 sm:px-4 text-left text-xs sm:text-sm">Qty</th>
                            <th className="py-2 sm:py-3 px-2 sm:px-4 text-left text-xs sm:text-sm">Price</th>
                            <th className="py-2 sm:py-3 px-2 sm:px-4 text-left text-xs sm:text-sm">Date</th>
                            <th className="py-2 sm:py-3 px-2 sm:px-4 text-left text-xs sm:text-sm">Status</th>
                            <th className="py-2 sm:py-3 px-2 sm:px-4 text-left text-xs sm:text-sm">Image</th>
                            {viewMode === "mine" && (
                        ...
                        <tbody>
                          {crops.map((crop) => (
                            ...
                            >
                              <td className="py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm">{crop.crop_id}</td>
                              <td className="py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm font-medium">{crop.crop_name}</td>
                              <td className="py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm">{crop.farmer_id}</td>
                              <td className="py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm">{crop.crop_type}</td>
                              <td className="py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm">{crop.quantity} kg</td>
                              <td className="py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm">{crop.price} TND/kg</td>
                              ...
                              {viewMode === "mine" && (
                                <td className="py-2 sm:py-3 px-2 sm:px-4">
                                  <div className="flex flex-col sm:flex-row gap-1 sm:gap-2">
                                    <button
                                      className="text-[#1d4c43] hover:text-[#2a5c45] text-xs sm:text-sm p-1"
                                      onClick={() => handleEdit(crop)}
                                    >
                                      Edit
                                    </button>
                                    <button
                                      className="text-red-500 hover:text-red-700 text-xs sm:text-sm p-1"
                ...
                <div className="text-center py-8 text-gray-500">
                  <p className="text-sm sm:text-base">No crops available.</p>
                </div>
              ))}
        ...
        {activeSection === "Farm Location" && (
          <div className={cardClass}>
            ...
            {/* Conteneur de carte avec hauteur responsive */}
            <div className="h-64 sm:h-80 lg:h-96 w-full border rounded-lg overflow-hidden">
