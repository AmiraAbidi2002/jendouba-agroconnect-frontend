...
// Available crop types for selection
const CROP_TYPES = [
  "food crops",
  "forage crops",
  "fiber crops",
...
export default function CropForm({ editingCrop, onSuccess, onCancel }) {
  ...
  // Populate form with crop data when editing
  useEffect(() => {
    if (editingCrop) {
      setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(error.message || "Error while saving crop. Please try again.");
    } finally {
    ...
    >
      ...
      <div className="flex items-center space-x-2">
        <input
          ...
          className="h-4 w-4"
        />
        <label htmlFor="availability" className="text-sm md:text-base">Available for sale</label>
      </div>
      ...
      <div>
        ...
        />
        console.log("Image URL:", crop.img_url);
      ...
      <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
        ...
        >
          {isSubmitting ? "Processing..." : editingCrop ? "Update Crop" : "Create Crop"}
        </button>
        <button
          type="button"
          onClick={onCancel}
