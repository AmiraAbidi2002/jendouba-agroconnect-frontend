...
export default function FarmerDashboard() {
  ...
  // États pour la gestion responsive
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      // Sur desktop, on force l'ouverture du menu
      if (desktop) {
        setIsMobileMenuOpen(true);
      } else {
        // Sur mobile, on ferme le menu si on redimensionne vers mobile
  ...
  const handleSectionChange = (section) => {
    ...
    // Fermer le menu mobile seulement si on est sur mobile
    if (!isDesktop) {
      setIsMobileMenuOpen(false);
    }
  };
  ...
  return (
    <div className="flex min-h-screen bg-[#f0f7f5]"> 
      ...
      `}>
        ...
        {!isDesktop && (
          <button
            className="self-end mb-4 text-white lg:hidden"
            onClick={() => setIsMobileMenuOpen(false)}
          >
            <FiX size={24} />
          </button>
