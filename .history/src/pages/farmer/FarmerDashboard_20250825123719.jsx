...
export default function FarmerDashboard() {
  ...
  return (
    <div className="flex min-h-screen bg-white">
      {/* Sidebar */}
      <aside className="w-64 bg-white text-gray-800 p-6 flex flex-col border-r border-gray-200">
        <h2 className="text-2xl font-bold mb-8 flex items-center">
          <span className="bg-[#1d4c43] text-white p-1 rounded mr-2">🌱</span> AgroConnect
        </h2>
        <nav className="flex-1">
          {menuItems.map(item => (
            <button
              key={item.name}
              className={`flex items-center w-full mb-2 px-4 py-3 rounded-lg transition-all ${activeSection === item.name ? "bg-[#1d4c43] text-white font-semibold shadow-md" : "hover:bg-gray-100 text-gray-800"}`}
        ...
        <div className="mt-auto pt-4 border-t border-gray-200 text-sm text-gray-500">Logged in as Farmer</div>
      </aside>
