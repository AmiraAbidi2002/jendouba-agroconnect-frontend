...
/**
 * CropForm Component
 * - Used for creating a new crop or editing an existing crop
 * - Validates input fields and handles file uploads
 * 
 * Props:
 * - editingCrop: crop object when editing
 * - onSuccess: callback when crop is successfully saved
 * - onCancel: callback when form is canceled
 */
export default function CropForm({ editingCrop, onSuccess, onCancel }) {
  ...
  // Populate form if editingCrop exists
  useEffect(() => {
    if (editingCrop) {
      setFormData({
  ...
  // Handle input changes for text, number, checkbox, and file
  ...
  const validateForm = () => {
    ...
    else if (formData.harvest_date < todayStr) newErrors.harvest_date = "Harvest date cannot be in the past";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      data.append("availability", formData.availability ? "true" : "false");
      ...
      alert(error.message || "Error saving crop. Check console for details.");
    } finally {
