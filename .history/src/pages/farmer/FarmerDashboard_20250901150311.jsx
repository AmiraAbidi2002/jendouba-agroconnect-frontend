...
export default function FarmerDashboard() {
  ...
  return (
    <div className="flex min-h-screen bg-[#f0f7f5]">
      {/* Bouton menu mobile */}
      <button
        className="lg:hidden fixed top-4 left-4 z-50 bg-[#1d4c43] text-white p-2 rounded-lg shadow-lg"
      ...
      {/* Sidebar */}
      <aside
        className={`fixed lg:relative w-64 h-full bg-[#1d4c43] text-white p-6 flex flex-col z-50 transform transition-transform duration-300 ease-in-out
          ...
          {menuItems.map((item) => (
            <button
              ...
              onClick={() => {
                ...
                setIsMobileMenuOpen(false); // fermer menu après clic sur mobile
              }}
            >
              {item.icon}
              {item.name}
