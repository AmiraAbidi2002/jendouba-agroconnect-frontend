...
export default function FarmerDashboard() {
  const myFarm = farms.find(f => f.ownerId === user.id);
<FarmMap farms={myFarm ? [myFarm] : []} />
  ...
  const [showAllFarms, setShowAllFarms] = useState(false); // Pour afficher toutes les fermes
  ...
  const fetchCrops = async () => {
    try {
      const data = await axios.get(`http://localhost:8080/crops/my?farmerId=${user.id}`);
      setCrops(data.data);
    } catch (error) {
  ...
  // Filtrer pour n'afficher que la ferme du farmer
      ...
      <main className="flex-1 p-8 overflow-auto">
        ...
        {activeSection === "Farm Location" && (
          <div className="bg-white rounded-xl shadow-md p-6">
            <h2 className="text-2xl font-bold mb-4">Farm Locations</h2>

            {/* Bouton pour voir toutes les fermes */}
            <button
