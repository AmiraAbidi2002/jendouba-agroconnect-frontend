...
export default function CropForm({ editingCrop, onSuccess }) {
  ...
  const validateForm = () => {
    ...
    if (!formData.harvest_date) newErrors.harvest_date = "Harvest date required";
    else {
      const selectedDate = new Date(formData.harvest_date);
      const today = new Date();
      today.setHours(0,0,0,0);
      if (selectedDate < today) newErrors.harvest_date = "Harvest date cannot be in the past";
    }
    setErrors(newErrors);
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <input
          ...
          className={`w-full border p-2 rounded ${
            errors.crop_name ? "border-red-500" : "border-gray-300"
          }`}
        />
        {errors.crop_name && (
          <p className="text-red-500 text-sm mt-1">{errors.crop_name}</p>
        )}
      </div>
      ...
      <div>
        ...
        />
        {errors.harvest_date && (
          <p className="text-red-500 text-sm mt-1">{errors.harvest_date}</p>
        )}
      </div>
