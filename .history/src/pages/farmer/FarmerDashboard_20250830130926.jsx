...
export default function FarmerDashboard() {
  ...
  // Memoize farms data to prevent unnecessary re-renders
  const farmsToShow = useMemo(() => farms, [farms]);

  // Fetch crops based on selected view mode (mine or all)
  ...
  // Fetch user's farm data and set profile information
  useEffect(() => {
    if (!user || !isFarmer) return;
  ...
  // Fetch all farms with their associated crops
  useEffect(() => {
    if (!isFarmer) return;
    ...
    const fetchFarms = async () => {
      try {
        ...
        // Fetch crops for each farm to display complete information
  ...
  // Load crops when active section changes to Crops
  useEffect(() => {
  ...
  // Save profile changes to both user and farm data
  const handleProfileSave = async () => {
    try {
      ...
      // Update farm information if exists
      if (mineFarm) {
      ...
      // Update authentication context with new user data
      if (userResponse.data) {
        // Preserve user role and ID from original user object
        const updatedUser = {
          ...userResponse.data,
      ...
      // Refresh farm data to ensure UI is up to date
      const farmData = await getMyFarm(user.id);
      setMineFarm(farmData);
      
      // Update the original profile data for cancel functionality
  ...
  const handleProfileCancel = () => {
    // Restore original data from before editing
  ...
  // Access control: only farmers can view this dashboard
  if (!user || !isFarmer) {
    return (
  ...
  return (
    <div className="flex min-h-screen bg-[#f0f7f5]">
      {/* Sidebar - Responsive: hidden on small screens, shown with menu toggle */}
      <aside className="hidden md:block w-64 bg-[#1d4c43] text-white p-6 flex flex-col">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-auto bg-[#f0f7f5]">
        {/* Mobile menu toggle button - Only shown on small screens */}
        <div className="md:hidden mb-4 flex justify-between items-center">
          <select 
            className="bg-[#1d4c43] text-white p-2 rounded"
            value={activeSection}
                ...
                {isEditingProfile ? (
                  <>
                    <button
                      ...
                      className="bg-[#1d4c43] text-white px-3 py-2 sm:px-4 sm:py-2 rounded hover:bg-[#2a5c45] flex items-center text-sm sm:text-base"
                    >
                      <FiSave className="mr-1 sm:mr-2" /> Save
                    </button>
                    <button
                      onClick={handleProfileCancel}
                      className="bg-gray-500 text-white px-3 py-2 sm:px-4 sm:py-2 rounded hover:bg-gray-600 flex items-center text-sm sm:text-base"
                    >
                      Cancel
                    </button>
                  </>
          ...
          <div className={cardClass}>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-2">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {viewMode === "mine" && (
                  <button
                    className="bg-[#1d4c43] text-white px-3 py-1 rounded hover:bg-[#2a5c45] text-sm sm:text-base"
                    onClick={() => {
                      setEditingCrop(null);
                      setShowForm(true);
                    }}
            ...
            {/* Crop list only if form is not open */}
            {!showForm &&
              (crops.length > 0 ? (
                <div className="overflow-x-auto mb-6">
                  <table className="min-w-full bg-white border border-gray-200 rounded-lg shadow">
                    <thead className="bg-[#1d4c43] text-white">
                      <tr>
                        <th className="py-3 px-2 sm:px-4 text-left">Crop ID</th>
                        <th className="py-3 px-2 sm:px-4 text-left">Name</th>
                        <th className="py-3 px-2 sm:px-4 text-left">Farmer ID</th>
                        ...
                        <th className="py-3 px-2 sm:px-4 text-left">Harvest Date</th>
                        <th className="py-3 px-2 sm:px-4 text-left">Availability</th>
                    ...
                    <tbody>
                      {crops.map((crop) => (
                        ...
                        >
                          <td className="py-3 px-2 sm:px-4">{crop.crop_id}</td>
                          <td className="py-3 px-2 sm:px-4">{crop.crop_name}</td>
                          <td className="py-3 px-2 sm:px-4">{crop.farmer_id}</td>
                          <td className="py-3 px-2 sm:px-4">{crop.crop_type}</td>
                          <td className="py-3 px-2 sm:px-4">{crop.quantity} kg</td>
                          <td className="py-3 px-2 sm:px-4">{crop.price} TND/kg</td>
                          ...
                          {viewMode === "mine" && (
                            <td className="py-3 px-2 sm:px-4 flex space-x-2">
                              <button
                                className="text-[#1d4c43] hover:text-[#2a5c45] text-sm"
