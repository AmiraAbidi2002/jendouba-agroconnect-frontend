...
// Ajouter cet useEffect pour détecter les changements de taille d'écran
useEffect(() => {
  const handleResize = () => {
    const mobile = window.innerWidth < 768;
    setIsMobile(mobile);
    // Sur desktop, on s'assure que la sidebar est ouverte
    if (!mobile) {
  ...
  // Ajouter ce useEffect pour fermer le sidebar quand on change de section sur mobile
useEffect(() => {
  if (isMobile) {
...
// Ajouter cette fonction pour toggle le sidebar
const toggleSidebar = () => {
  ...
  useEffect(() => {
    ...
    const fetchFarm = async () => {
      try {
        ...
        console.error("Error getMyFarm:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  {isSidebarOpen &&  isMobile &&(
    ...
    />
        ...
        <img
          ...
          className="mr-2 rounded-full object-cover md:block"
