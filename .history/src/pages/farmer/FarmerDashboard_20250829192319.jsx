...
export default function FarmerDashboard() {
  ...
  // ----- Fetch crops en fonction du mode choisi -----
  ...
  useEffect(() => {
    ...
    const fetchFarms = async () => {
      try {
        ...
        // Pour chaque ferme, récupérer les crops
  ...
  // Charger les crops quand la section active change
  useEffect(() => {
  ...
  const handleProfileSave = async () => {
    try {
      ...
      // Mettre à jour les informations de la ferme si elle existe
      if (mineFarm) {
      ...
      // Mettre à jour le contexte d'authentification avec les données correctes
      if (userResponse.data) {
        // Préserver le rôle et l'ID de l'utilisateur
        const updatedUser = {
          ...userResponse.data,
          role: user.role, // Garder le rôle original
          id: user.id // Garder l'ID original
        };
        updateUser(updatedUser);
      }
      
      // Rafraîchir les données de la ferme
      ...
      <main className="flex-1 p-8 overflow-auto bg-white">
        ...
        {activeSection === "Crops" && (
          <div className={cardClass}>
            ...
            {/* Liste des crops seulement si le formulaire n'est pas ouvert */}
            {!showForm &&
              (crops.length > 0 ? (
            ...
            {/* Formulaire Crop */}
            {showForm && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50  crop-form-modal">
                <div className="bg-[#1d4c43] text-white rounded-xl shadow-lg 
            w-full max-w-2xl relative p-6 flex flex-col items-center justify-center">
                  <button
                    onClick={() => {
                      setShowForm(false);
                      setEditingCrop(null);
                    }}
                    className="bg-[#1d4c43] text-white px-3 py-1 rounded relative flex flex-col items-center justify-center"
                  >
                    <FiX size={24} />
                  </button>
                  <CropForm
