    ...
    <div className="flex flex-row h-screen bg-[#f0f7f5]"> 
      ...
      {isMobile && (
        <>
          ...
          {isSidebarOpen && (
            <aside
              className="fixed top-0 left-0 h-full w-64 bg-[#1d4c43] text-white p-6 z-40 overflow-auto"
            >
              <img
                src="/src/assets/jendouba_agroconnect_logo.png"
                alt="Logo AgroConnect"
              ...
              <nav className="flex-1">
                {menuItems.map((item) => (
                  <button
                    key={item.name}
                    className={`fixed flex items-center w-full mb-2 px-4 py-3 transition-colors rounded-lg ${
                      activeSection === item.name
                        ? "text-orange-500 bg-white bg-opacity-10"
                        : "text-white hover:bg-white hover:bg-opacity-5"
                    }`}
