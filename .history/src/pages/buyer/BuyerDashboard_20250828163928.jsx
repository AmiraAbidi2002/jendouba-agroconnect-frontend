...
export default function BuyerDashboard() {
  ...
  const parseLatLng = (loc) => {
    try {
      ...
      if (typeof loc === 'string') {
        ...
        // Already an object with lat/lng properties
        return { lat: parseFloat(loc.lat), lng: parseFloat(loc.lng) };
      }
      
      return { lat: 36.4366, lng: 8.4518 }; // fallback
    } catch {
      return { lat: 36.4366, lng: 8.4518 }; // fallback
    }
  };
  ...
  useEffect(() => {
    ...
    if (user) { 
      console.log("User object:", user); // Debug: voir la structure de l'utilisateur
      const loc = parseLatLng(user.location);
      
      setProfileData({
        id: user.sub || user.id || "", // Priorité à sub (venant du JWT)
  ...
  const handleProfileSave = async () => {
    ...
    try {
      // Get the correct user ID - Vérifions d'abord la structure de l'utilisateur
      console.log("User object on save:", user); // Debug
  ...
  // Correction du filtre par date de récolte
                ...
                <tbody className="text-gray-600 text-sm">
                  {filteredCrops.length > 0 ? (
                    filteredCrops.map((crop) => (
                      <tr key={crop.id} className="border-b border-gray-200 hover:bg-gray-50">
                        ...
                        <td className="py-3 px-6">{crop.farmer_name}</td>
                        <td className="py-3 px-6">{crop.description || 'No description'}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
