...
export default function BuyerDashboard() {
  ...
  try {
    if (!url || !url.includes("?q=")) return { lat: 0, lng: 0 };
  ...
  useEffect(() => {
    ...
    if (user) { 
      const loc = parseLatLngFromUrl(user.location);
      setProfileData({
  ...
  const handleProfileSave = async () => {
    ...
    try {
      ...
      // Prepare data for API call (don't send password if it's the masked value)
       ...
       // Convertir lat/lng en URL pour le backend
    const locationUrl = `https://www.google.com/maps?q=${profileData.location.lat},${profileData.location.lng}`;
                ...
                <div className="h-64 w-full border rounded-lg overflow-hidden">
                  <FarmMap 
                    farms={[{
                      ...
                      locationUrl: `https://www.google.com/maps?q=${profileData.location?.lat || 0},${profileData.location?.lng || 0}`,
                      crops: []
                    }]} 
                    height={256}
