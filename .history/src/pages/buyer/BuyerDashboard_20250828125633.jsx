...
export default function BuyerDashboard() {
  ...
  useEffect(() => {
    ...
    if (user) {
      const userLocation = user.location || determineUserLocation();
  ...
  const determineUserLocation = () => {
    // Try to get location from browser if available
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          // Default to Jendouba coordinates if geolocation fails
          return { lat: 36.5, lng: 8.8 };
        }
      );
    } else {
      // Default to Jendouba coordinates if geolocation is not supported
      return { lat: 36.5, lng: 8.8 };
    }
  };
