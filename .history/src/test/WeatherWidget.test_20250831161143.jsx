...
const mockWeatherData = {
  ...
  daily: Array.from({ length: 7 }, (_, i) => ({
    ...
    weather: [{ description: i % 2 === 0 ? "cloudy" : "sunny", icon: i % 2 === 0 ? "02d" : "01d" }],
    main: { temp: 20 + i },
  })),
};
...
describe("WeatherWidget", () => {
  ...
  test("permet de minimiser et maximiser la section Today", async () => {
    ...
    const button = await screen.findByRole("button", { name: /toggle today section/i });
  ...
  test("appelle l'API avec les bonnes coordonnées", async () => {
    getWeatherByCoords.mockResolvedValueOnce(mockWeatherData);
    render(<WeatherWidget />);
    
    await waitFor(() => {
      expect(getWeatherByCoords).toHaveBeenCalledWith(expect.any(Number), expect.any(Number));
    });
  });
});