...
const server = setupServer(
  http.get("http://localhost:8080/api/weather", (req, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.json({
        temperature: 25,
...
test("WeatherWidget affiche la météo correctement", async () => {
  ...
  const today = await screen.findByText(/today/i);
  expect(today).toBeInTheDocument();

  // On limite la recherche au bloc Today pour éviter les doublons de "25°C"
