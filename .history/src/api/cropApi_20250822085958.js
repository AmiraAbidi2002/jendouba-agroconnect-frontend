...
export const createCrop = async (formData) => {
  try {
    ...
    // Axios détecte automatiquement multipart/form-data pour FormData
...
// Récupérer les cultures d'un agriculteur
export const getCropsByFarmer = async (farmerId) => {
  try {
    const response = await api.get(`api/crops/farmer/${farmerId}`);
    return response.data;
  } catch (error) {
