      ...
      {selectedContact && (showConversation || !isMobileView) && (
        <div className="w-full md:w-2/3 flex flex-col bg-white">
          ...
          <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-4 bg-white">
            <div className="flex flex-col"> {/* Retirer gap et utiliser margin directement */}
              {messages.map((m) => {
                const isMine = m.senderId === user.id;
                return (
                  <div 
                    key={m.msg_id || `${m.timestamp}-${Math.random()}`} 
                    className={`flex ${isMine ? "justify-end" : "justify-start"} mb-4`} // Ajout de mb-4 pour marge basse
                  >
                    <div
