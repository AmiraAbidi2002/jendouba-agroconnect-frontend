...
export default function MessageList({ user, contacts = [] }) {
  ...
  // Filtrer les contacts qui ont des messages
  const contactsWithMessages = contacts.filter(contact => 
    messages.some(msg => 
      msg.senderId === (contact.id || contact.user_id) || 
      msg.receiverId === (contact.id || contact.user_id)
    )
  );
      ...
      >
        {selectedContact ? (
          ...
          <div className="h-full flex flex-col items-center justify-center text-gray-400 text-center p-6">
            ...
            <p className="text-sm text-gray-500 mb-4">Sélectionnez une conversation pour commencer</p>
          </div>
        )}
      </div>
    </div>
