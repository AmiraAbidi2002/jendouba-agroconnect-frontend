...
export default function MessageList({ user, contacts = [] }) {
    ...
    try {
      ...
      msgs = msgs.map(msg => {
        if (Array.isArray(msg.timestamp)) {
          // Convertir le tableau [2025, 8, 25, 11, 30, 41, 134247000] en Date
          const [year, month, day, hours, minutes, seconds, milliseconds] = msg.timestamp;
          msg.timestamp = new Date(year, month - 1, day, hours, minutes, seconds, milliseconds / 1000000);
  ...
  const handleSearchSubmit = (e) => {
    ...
    const existing = contacts.find(
      (c) => (c.id || c.user_id).toString() === searchTerm.trim()
    );
    if (existing) {
      setSelectedContact(existing);
    } else {
      const newContact = { user_name: `User ${searchTerm}`, id: searchTerm };
  ...
  const formatTimestamp = (timestamp) => {
    ...
    if (timestamp instanceof Date) {
      ...
      console.warn("Format de date non supporté:", timestamp);
  ...
  return (
    <div className="flex h-full rounded-lg shadow-md overflow-hidden border border-gray-200" style={{ minHeight: 480 }}>
      ...
      {/* Conversation */}
      {selectedContact && (showConversation || !isMobileView) && (
        <div className="w-full md:w-2/3 flex flex-col bg-white">
          ...
          <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-4 bg-white">
            <div className="space-y-4"> {/* Augmentation de l'espacement entre les messages */}
              {messages.map((m) => {
                const isMine = m.senderId === user.id;
                return (
                  <div key={m.msg_id || `${m.timestamp}-${Math.random()}`} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                    <div
                      className="px-4 py-2 rounded-2xl shadow-sm break-words max-w-xs"
                      style={{
                        backgroundColor: isMine ? MESSAGE_BG : "#F0F0F0",
