...
export default function MessageList({ user }) {
  ...
  // Rafraîchir messages toutes les 3s
  useEffect(() => {
    if (selectedContact) {
      fetchMessages();
  ...
  // Fermer résultats recherche si clic dehors
  useEffect(() => {
    ...
    <div className="flex h-full w-full rounded-lg shadow-md overflow-hidden border border-gray-200 bg-white" style={{ minHeight: "600px", maxHeight: "600px" }}>
      ...
      <div className={`${showConversation ? 'hidden md:flex' : 'flex'} w-full md:w-1/3 border-r border-gray-200 flex-col bg-white`}>
        ...
        <div className="p-3 flex-shrink-0 relative" ref={searchRef}>
          ...
          {/* Résultats recherche avec background opaque */}
          {showSearchResults && (
            <div className="absolute top-full left-3 right-3 mt-1 max-h-60 overflow-y-auto bg-white border border-gray-300 rounded-lg shadow-xl z-50" 
              ...
              {searchResults.length > 0 ? (
                searchResults.map((u) => (
                  ...
                  >
                    <div className="flex-1">
                      <div className="font-medium text-sm truncate text-gray-800">{u.user_name}</div>
                ...
                <div className="p-3 text-gray-600 text-sm text-center" style={{ backgroundColor: '#ffffff' }}>
        ...
        <div className="flex-1 overflow-y-auto">
          {conversations.length === 0 ? (
            <div className="text-center text-gray-500 p-4 text-sm">Aucune conversation</div>
          ) : (
            <div className="p-2">
              {conversations.map((c) => {
      ...
      {showConversation && selectedContact && (
        <div className="flex-1 flex flex-col bg-white min-w-0 h-full">
          ...
          {/* Messages - ZONE SCROLLABLE AMÉLIORÉE */}
          <div
              ...
              {messages.length === 0 ? (
                ...
                messages.map((m) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`relative max-w-[70%] px-4 py-3 rounded-2xl shadow-sm border-2 ${
                          isMine
                            ? "rounded-br-sm bg-opacity-95"
                            : "rounded-bl-sm"
                        }`}
                        style={{
                          backgroundColor: isMine ? SIDEBAR_COLOR : "#ffffff",
                          color: isMine ? "#ffffff" : "#2d3748",
                          borderColor: isMine ? SIDEBAR_COLOR : "#e2e8f0",
                          boxShadow: isMine 
                            ? "0 4px 6px -1px rgba(29, 76, 67, 0.3)" 
                      ...
                      >
                        ...
                        <div
                          className={`absolute top-3 w-0 h-0 ${
                            isMine 
                              ? "right-[-8px] border-l-[8px] border-t-[8px] border-b-[8px]" 
                              : "left-[-8px] border-r-[8px] border-t-[8px] border-b-[8px]"
                          }`}
                          style={{
                            borderColor: isMine 
                              ? `transparent transparent transparent ${SIDEBAR_COLOR}`
                              : `transparent ${SIDEBAR_COLOR} transparent transparent`,
                        ...
                        {/* Timestamp */}
                        <div 
                          className={`text-xs mt-2 ${isMine ? 'text-right' : 'text-left'}`}
                          style={{ 
                            color: isMine ? "rgba(255, 255, 255, 0.7)" : "#718096",
                            fontSize: "11px"
                          }}
                        >
