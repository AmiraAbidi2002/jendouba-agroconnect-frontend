...
export default function WeatherWidget() {
  ...
  return (
    <div className="space-y-6 px-4 md:px-8 py-6">
      ...
      {/* Today Forecast centré avec largeur réduite */}
      <div className="flex justify-center mb-6">
        <div
          className={`relative w-full max-w-xs bg-[#1d4c43] text-white p-4 rounded-2xl shadow-lg transition-all duration-300 ${minimized ? 'minimized-width' : ''}`}
        >
          <button 
            onClick={toggleMinimize}
            className="absolute top-3 right-3 bg-white bg-opacity-30 rounded-full w-7 h-7 flex items-center justify-center text-gray-700 hover:bg-opacity-50 transition-all"
          >
            {minimized ? '+' : '-'}
          </button>

          <h2 className="text-lg font-semibold text-center">Aujourd'hui</h2>

          {!minimized && (
            <div className="today-details mt-3">
              <div className="text-4xl md:text-5xl font-bold mb-2 text-center">
                {Math.round(current.main.temp)}°C
              </div>
              <div className="capitalize mb-4 text-center text-sm">{current.weather[0].description}</div>
