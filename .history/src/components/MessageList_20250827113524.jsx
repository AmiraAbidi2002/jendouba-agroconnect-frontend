...
import { FiSearch, FiMessageSquare, FiArrowLeft, FiSend, FiUser, FiVideo, FiUserPlus } from "react-icons/fi";
...
export default function MessageList({ user, contacts = [] }) {
  ...
  // Simuler des statuts en ligne/aléatoires pour les contacts
  useEffect(() => {
    const statuses = {};
    contacts.forEach(contact => {
      statuses[contact.id || contact.user_id] = Math.random() > 0.3;
    });
        ...
        <div className="flex-1 overflow-y-auto bg-white">
          {filteredContacts.length > 0 ? (
            filteredContacts.map((contact) => {
              const isOnline = onlineStatus[contact.id || contact.user_id];
          ...
          <>
            <div className="p-4 border-b border-gray-200 bg-white flex items-center justify-between shadow-sm">
              <div className="flex items-center">
                ...
                <div className="relative flex-shrink-0">
                  ...
                  {onlineStatus[selectedContact.id || selectedContact.user_id] && (
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.length > 0 ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-gray-400 text-center p-6">
                  <FiMessageSquare className="text-4xl mb-3 text-gray-300" />
            ...
            <div className="p-4 border-t border-gray-200 bg-white flex items-center">
              <input
                ...
                className="flex-1 border border-gray-300 rounded-l-full px-4 py-2 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent bg-white"
              />
              <button
                onClick={handleSend}
                disabled={!newMsg.trim()}
                className="bg-green-600 text-white px-5 py-2 rounded-r-full hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center transition-colors"
              >
