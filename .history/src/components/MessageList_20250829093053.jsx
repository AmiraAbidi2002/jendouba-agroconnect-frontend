...
export default function MessageList({ user }) {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:8080/messages/${user.id}/${selectedContact.id}`
      );
      let msgs = res.data || [];
...
async function searchUsers(query) {
  ...
  return res.data; // tableau d’utilisateurs {user_id, user_name, ...}
}
  ...
  const handleSearchSubmit = async (e) => {
    ...
    let existing = conversations.find(
      (c) =>
        ...
        c.user_name.toLowerCase() === searchTerm.trim().toLowerCase()
    );
    ...
    if (!existing) {
      try {
        ...
        if (res.data && res.data.length > 0) {
          const found = res.data[0]; // 👉 Prend le premier résultat
          existing = { id: found.user_id, user_name: found.user_name};
        }
      } catch {
