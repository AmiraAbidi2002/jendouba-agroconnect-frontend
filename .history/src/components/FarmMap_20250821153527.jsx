...
/**
 * props.farms : tableau d’objets
 * {
 *   farmerId, farmerName, locationUrl, lat, lng,
 *   crops: [{ id, name, type, quantity, price, availability }]
 * }
 */
export default function FarmMap({ farms = [], height = 420 }) {
  const center = useMemo(() => {
    // centre par défaut Jendouba
    return farms.length
  ...
  return (
    <div style={{ height }}>
      <MapContainer center={center} zoom={9} style={{ height: "100%", width: "100%" }}>
        ...
        />
        {farms.map((f) => (
          <Marker key={f.farmerId} position={[f.lat || 0, f.lng || 0]}>
            <Popup>
                ...
                <div className="mt-2">
                  ...
                  {f.crops?.slice(0, 5).map((c) => (
                    <div key={c.id} className="text-sm">
                      • {c.name} ({c.type}) — {c.quantity} kg @ {c.price} TND
