...
const SENT_BG = SIDEBAR_COLOR; // messages envoyés
const RECEIVED_BG = "#F0F0F0"; // messages reçus
          ...
          <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-4 bg-white" style={{ maxHeight: 'calc(100vh - 200px)', minHeight: '300px' }}>
            <div className="flex flex-col" style={{ gap: '16px' }}>
              {messages.map(m => {
                ...
                return (
                  <div key={m.msg_id || `${m.timestamp}-${Math.random()}`} className={`flex ${isMine ? "justify-end" : "justify-start"}`} style={{ marginBottom: '16px' }}>
                    <div
