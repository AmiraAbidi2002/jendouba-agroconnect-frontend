    ...
    <div className="flex h-full w-full rounded-lg shadow-md overflow-hidden border border-gray-200 bg-white" style={{ minHeight: "600px" }}>
                ...
                searchResults.map((u) => (
                  ...
                  >
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-gray-500 truncate">ID: {u.user_id}</div>
                    </div>
                    <button
      ...
      {showConversation && selectedContact && (
        <div className="flex-1 flex flex-col bg-white min-w-0">
          ...
          {/* Messages */}
          <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-4 flex flex-col bg-gray-50">
            {messages.length === 0 ? (
              <div className="text-center text-gray-500 p-4 text-sm">Aucun message dans cette conversation</div>
            ) : (
              messages.map((m) => {
                ...
                return (
                  <div
                    ...
                    className={`max-w-[70%] mb-2 px-4 py-2 rounded-lg break-words whitespace-pre-wrap ${
                      isMine ? "self-end bg-orange-100 text-orange-800 text-right" : "self-start bg-blue-100 text-blue-800 text-left"
                    }`}
                  >
                    <p>{m.content}</p>
                    <div className="text-xs mt-1 opacity-60">{formatTimestamp(m.timestamp)}</div>
                  </div>
                );
              })
            )}
