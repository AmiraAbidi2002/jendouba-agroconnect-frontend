...
export default function MessageList({ user, contacts = [] }) {
  ...
  const SIDEBAR_ITEM_BG = "#FEF2F2"; // Nouvelle couleur pour les contacts
  ...
  const handleAddConversation = () => {
    ...
    // Simule la sélection de la nouvelle conversation
    setSelectedContact({ user_id: newReceiverId, user_name: `User ${newReceiverId}`, role: "Collaborateur" });
    setNewReceiverId("");
  ...
  return (
    <div className="flex h-full rounded-lg shadow-md overflow-hidden border border-gray-200" style={{ minHeight: 480 }}>
      ...
      <div className={`w-full md:w-2/3 flex flex-col bg-white ${isMobileView && !showConversation ? "hidden" : "flex"}`}>
        {selectedContact ? (
          <>
            {/* Header de conversation avec couleur sidebar */}
            ...
            {/* Zone des messages avec scroll limité */}
            <div 
      ...
      {/* Modale d'ajout de conversation */}
      {showAddModal && (
