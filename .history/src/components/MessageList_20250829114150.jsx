...
export default function MessageList({ user }) {
  ...
  const fetchConversations = useCallback(async () => {
    ...
    try {
      ...
      // Essayer d'abord l'endpoint conversations
      try {
        ...
        // Si l'endpoint conversations n'existe pas, essayer de récupérer tous les messages de l'utilisateur
        if (convErr.response?.status === 404) {
          console.log("Endpoint conversations non disponible, récupération via messages...");
          try {
            const messagesRes = await axios.get(
              `http://localhost:8080/api/messages/user/${user.id}`,
              {
                headers: {
                  Authorization: `Bearer ${token}`,
                },
            ...
            // Créer la liste des conversations à partir des messages
            ...
            messages.forEach(msg => {
              ...
              if (!conversationsMap.has(contactId)) {
                conversationsMap.set(contactId, {
                  id: contactId,
                  user_name: msg.senderName || msg.receiverName || `Utilisateur ${contactId}`,
            ...
            console.error("Erreur récupération messages pour conversations :", msgErr);
            setConversations([]);
          }
        } else {
          throw convErr;
  ...
  const handleSearchSubmit = async (e) => {
    ...
    try {
      ...
      // Essayer d'abord l'endpoint de recherche
      try {
        const res = await axios.get(
        ...
        // Si l'endpoint de recherche n'existe pas, essayer de récupérer tous les utilisateurs
        if (searchErr.response?.status === 404) {
          console.log("Endpoint de recherche non disponible, tentative avec endpoint utilisateurs...");
          try {
            ...
            // Filtrer localement par nom ou ID
            const allUsers = allUsersRes.data || [];
            const searchTermLower = searchTerm.toLowerCase();
            const filteredResults = allUsers.filter(u => 
              u.user_id !== user.id && (
                u.user_name?.toLowerCase().includes(searchTermLower) ||
                u.user_id?.toString().includes(searchTerm)
              )
            );
            setSearchResults(filteredResults);
          } catch (allUsersErr) {
            console.error("Erreur récupération de tous les utilisateurs :", allUsersErr);
            setSearchResults([]);
          }
        } else {
          throw searchErr;
        ...
        <div className="flex-1 overflow-y-auto">
          {conversations.length === 0 ? (
            <div className="text-center text-gray-500 p-4 text-sm">
              <div className="mb-2">Aucune conversation</div>
              <div className="text-xs">Utilisez la recherche pour trouver des utilisateurs</div>
            </div>
          ) : (
            <div className="p-2">
              {conversations.map((c) => {
                    ...
                    >
                      <div
                        className={`max-w-[75%] p-3 rounded-lg shadow-sm ${
                          isMine 
                            ? "bg-orange-100 rounded-br-sm ml-auto" 
                            : "bg-blue-100 rounded-bl-sm mr-auto"
                        }`}
                        style={{
                      ...
                      >
                        <p className="text-sm break-words whitespace-pre-wrap" style={{ margin: 0 }}>
                          {m.content}
                        </p>
                        <div 
                          className={`text-xs mt-1 ${isMine ? "text-orange-600 text-right" : "text-blue-600 text-left"}`}
                          style={{ marginTop: '4px' }}
                        >
