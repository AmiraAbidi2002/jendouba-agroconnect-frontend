...
export default function MessageList({ user }) {
  ...
  const handleSearchSubmit = async (e) => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:8080/users/search?q=${encodeURIComponent(searchTerm.trim())}`
      );
        ...
        <div className="w-full flex flex-col bg-white">
              ...
              {messages.map((m) => {
                ...
                return (
                  <div key={m.msg_id || `${m.timestamp}-${Math.random()}`} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-xs px-4 py-2 rounded-2xl shadow-sm break-words ${isMine ? "bg-[#1d4c43] text-white rounded-br-none" : "bg-gray-100 text-gray-800 rounded-bl-none"}`} style={{ border: `1px solid ${isMine ? "#1d4c43" : "#e5e7eb"}` }}>
          ...
          <div className="p-3 border-t border-gray-200 flex gap-2 items-center bg-white">
            <textarea
              ...
              rows={1}
            />
            <button onClick={handleSend} className="px-4 py-2 rounded-lg text-white font-semibold" style={{ backgroundColor: SEND_BTN_ORANGE }}>
              <FiSend size={20} />
            </button>
          </div>
        </div>
