...
export default function MessageList({ user }) {
  ...
  useEffect(() => {
    if (!user) return;
    const typeToFetch = user.role === "BUYER" ? "FARMER" : "BUYER";
    axios
  ...
  <div className="col-span-2 border p-2 flex flex-col">
    <ul className="flex-1 overflow-auto space-y-2">
      {messages.map(m => (
        <li key={m.msg_id} className={`${m.sender.id === user.id ? "text-blue-600 text-right" : "text-black text-left"}`}>
          <span className="font-semibold">{m.sender.user_name}: </span>{m.content}
        </li>
      ))}
    </ul>
    ...
    <div className="mt-2 flex">
      ...
      />
      <button onClick={handleSend} className="ml-2 bg-green-600 text-white px-2 py-1 rounded">
        Envoyer
      </button>
    </div>
  </div>
