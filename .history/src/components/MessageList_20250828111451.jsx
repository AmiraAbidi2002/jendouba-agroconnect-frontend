  ...
  const formatTimestamp = (timestamp) => {
    ...
    if (timestamp instanceof Date) {
      ...
      if (isNaN(dateObj.getTime())) {
        // Si le timestamp est invalide, essayer d'autres formats
        if (typeof timestamp === 'string' && timestamp.includes('T')) {
          // Essayer de parser les dates ISO sans les millisecondes
          const isoDate = timestamp.includes('.') ? timestamp : timestamp + '.000Z';
          dateObj = new Date(isoDate);
        }
        
        // Si toujours invalide, utiliser la date actuelle
        if (isNaN(dateObj.getTime())) {
