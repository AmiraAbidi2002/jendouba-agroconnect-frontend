          ...
          {filteredContacts.map((contact) => {
            ...
            return (
              <div
                ...
                className={`p-2 m-2 rounded-lg cursor-pointer transition-colors bg-[#FEEAA1] border border-gray-300 ${isSelected ? "ring-2 ring-orange-400" : "hover:ring-1 hover:ring-orange-200"}`}
              ...
              {messages.map((message) => (
                <div key={message.msg_id} className={`flex ${message.senderId === user.id ? "justify-end" : "justify-start"}`}>
                  <div className="max-w-xs md:max-w-md">
                    <div
                      className={`px-4 py-2 rounded-xl border ${message.senderId === user.id ? "border-orange-300 bg-[#FEEAA1]" : "border-gray-300 bg-white"}`}
                    >
                      <p className="text-sm break-words">{message.content}</p>
                      <p className="text-[10px] mt-1 text-right text-gray-500">{formatMessageTime(message.timestamp)}</p>
                    </div>
                  </div>
                </div>
              ))}
